...
!
! MAIN PROGRAM: HRDPS2ARL     DECODE HRDPS GRIB MODEL FIELDS FOR HYSPLIT
! PRGMMR: JN CANDAU           ORG: NRCAN/CFS       DATE: 2025-11-20
!
!-------------------------------------------------------------
! ABSTRACT:  THIS CODE IS A FORK FROM CODE
!                        WRITTEN AT THE AIR RESOURCES LABORATORY ...
!
! Converts ENVIRONMENT CANADA HRDPS data using ECMWF ecCodes library
...
!    
! a default hrdps2arl.cfg will be created if none exists or
...
!                                  level grib file.
!               20 NOV 2025(JNC) - Copy era52arl.f and customize for
!               HRDPS 
    
!
! Input files 
! grib file with pressure levels
! grib file with analysis surface fields
! grib file with forecast surface fields
! hrdps2arl.cfg file detailing fields in the grib files
!
!-------------------------------------------------------------
!This program will currently only convert HRDPS data on the pressure levels.

!Pressure levels to use in the file are specified in the hrdps2arl.cfg file.
...
!Read the  shortname, parameterCategroy, and level using grib_get.
!Loop through all messages in the 2D analysis file if it exists. 
!Read the  shortname, parameterCategory, and level using grib_get.
...
PROGRAM hrdps2arl 
  ...
  real            ,dimension(maxvar) :: atmcnv, sfccnv ! conversion factor HRDPS->ARL
  character(len=6),dimension(maxvar) :: atmgrb, sfcgrb ! HRDPS variable short names
...
! 
  ...
  open(kunit,file='HRDPS2ARL.MESSAGE')  ! open message file
     ...
     SELECT CASE(ftype(num_files - iii+1))
        ...
        CASE (3)  !3d model or pressure level file. This case evaluated last.
         ...
         DO i=1,num_msg  !loop through 3D file messages
            ...
            !Check that we have a pressure level variable
            IF(trim(ltype).EQ.'pl') THEN
               call grib_get(igrib(i),'shortName',value)
               call grib_get(igrib(i),'parameterCategory',pcat)
               kv=-1
               !kv is the index of the variable in the config file
               do k=1,numatm
                  if (pcat.eq.atmcat(k).and.trim(value).eq.atmgrb(k)) kv=k
               enddo

               ! If the variable is present in the config file
               IF(kv.NE.-1)THEN
                  call grib_get(igrib(i),'level',levhgt)
                  write(kunit,*) 'levelsA', trim(value), pcat,   levhgt
                  kl=-1
                  ! If the level of the variable is in the config file
                  DO k=1,numlev
                     IF(ndxlevels(k).eq.levhgt) THEN
                        kl=k 
                     END IF
                  END DO
                  if (kl.eq.-1) kv=-1
               ...
               END IF  !if kv
            ELSE
                WRITE(kunit,*)'levelType not pl for message: ',i,ltype,'X trim(ltype):',trim(ltype),'X'
...
! grid geolocation parameters and projection
  DO N=1,12
     ![JNC] changed from F10.2 to F10.4
...
!-------------------------------------------------------------
! Create the configuration file if the file 
! does not already exist. Use GRIB shortName, category and parameter.
!
! HRDPS variables vs Hysplit:
! See: https://eccc-msc.github.io/open-data/msc-data/nwp_hrdps/readme_hrdps-datamart_en/
! and: https://www.ready.noaa.gov/hysplitusersguide/S141.htm
!
! 2D analysis variables
! prmsl - mean sea level pressure (Pa in HRDPS, hPa in Hysplit)
! 10u   - 10 metre U wind component (m/s)
! 10v   - 10 metre V wind component (m/s)
! 2t    - 2 metre temperature (K)
! blh   - PBL height (m)
! sp    - pressure (Pa in HRDPS, hPa in Hysplit) 
! ishf  - sensible heat flux (W/m2)
...
!
! 3D variables
! gh     - geopotential (gpm in HRDPS, it seems to be the same in Hysplit)
! t      - temperature (K in HRDPS, same in Hysplit)
! u      - u wind (m/s in HRDPS, same in Hysplit)
! v      - v wind (m/s in HRDPS, same in Hysplit)
...
SUBROUTINE makapi(apicfg_name)
  ...
  write(30,'(a)')' numatm = 7,'
  write(30,'(a)')' atmgrb = '//a//'gh'//d//'t'//d//'u'//d//'v'//d//'w'//d//'r'//d//'q'//a//c
